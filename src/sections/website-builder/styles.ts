export const styles = {
  ...
  notification: {
    ...
    badge:
      "flex items-center gap-2 py-2 px-3 rounded-xl lg:rounded-full bg-slate-200 dark:bg-slate-700",
    icon: "w-4 h-4 lg:w-5 lg:h-5",
    text: "font-medium text-xs lg:text-sm",
    cta: "font-medium text-xs flex gap-2 pb-2 lg:pb-0 lg:text-sm text-slate-700 dark:text-white dark:group-hover:text-white group-hover:text-slate-900",
    arrow:
      "w-4 h-4 lg:w-5 lg:h-5 group-hover:translate-x-1 transition-transform duration-200",
  },
  ...
  buttons: {
    ...
    arrow:
      "w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform duration-200",
  },

  statistics: {
    container: "flex justify-evenly py-4 w-fit gap-4 lg:gap-8",
    item: "text-center lg:text-left px-2 flex flex-col gap-2",
    value:
      "text-xl sm:text-3xl lg:text-4xl font-bold text-slate-900 dark:text-white",
    label: "text-slate-600 dark:text-slate-400 text-xs sm:text-sm lg:text-base",
  },

  content: {
    container: "z-10 flex flex-col w-full items-center justify-center lg:items-start px-4 pt-[14dvh] lg:pt-0 lg:px-[10%]",
    header: "text-center lg:text-left h-full w-full lg:w-[60%] flex flex-col items-center justify-center lg:items-start gap-8",
    headline:
      "text-3xl lg:text-5xl xl:text-7xl font-bold text-slate-900 dark:text-white leading-tight",
    description:
      "text-slate-600 dark:text-slate-300 text-sm sm:text-lg lg:text-xl max-w-2xl lg:max-w-lg leading-relaxed",
  },
